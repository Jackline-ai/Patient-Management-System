...
public class ServiceItemsService {
	...
	try {
	    ...
	    ServiceRegistrationEvent event = new ServiceRegistrationEvent(saved.getServiceId(), saved.getServiceName(),
		    saved.getQuantity(), saved.getPrice(), saved.getDiscount(), saved.getTotal(), "CREATE");
	...
	try {
		...
		PatientServiceItems serviceItem = service.get();

		ServiceRegistrationEvent event = new ServiceRegistrationEvent();
		event.setServiceId(service.get().getServiceId());
		event.setEventType("DELETE");
		event.setServiceName(service.get().getServiceName());

		serviceKafkaProducer.SendServiceRegistration(event);

		serviceItemsRepository.delete(serviceItem);
	    ...
	    e.printStackTrace();
	    throw new RuntimeException("error deleting service");
	}
    }
